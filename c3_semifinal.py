...
def generate_pdf(itinerary, destination, trip_details, attractions, itinerary_number=1):
    ...
    pdf.cell(0, 8, f"Interests: {trip_details['interests']}", new_x="LMARGIN", new_y="NEXT")
    ...
    if attractions:
        ...
        for attraction in attractions[:5]:
            ...
            pdf.cell(0, 8, attraction['name'], new_x="LMARGIN", new_y="NEXT")
            pdf.set_font("helvetica", "", 10)
            pdf.cell(0, 6, f"Type: {attraction['type']} | Rating: {attraction['rating']}", new_x="LMARGIN", new_y="NEXT")
            pdf.cell(0, 6, f"Address: {attraction['address']}", new_x="LMARGIN", new_y="NEXT")
    ...
    for para in paragraphs:
        ...
        if lines and "Day" in lines[0]:
            ...
            pdf.cell(0, 10, day_title, new_x="LMARGIN", new_y="NEXT")
            if image_index < len(city_images) and pdf.get_y() + 60 < pdf.h - 15:
                try:
                    ...
                    pdf.cell(0, 5, f"Scene {image_index}: {destination}", new_x="LMARGIN", new_y="NEXT", align="C")
                    os.remove(img_path)
                    image_index += 1
                except:
                    pass
            ...
            if '<a href="' in line:
                ...
                for i, part in enumerate(parts):
                    if i == 0:
                        ...
                        pdf.set_text_color(0, 0, 255)
                        pdf.multi_cell(0, 8, text, link=url)
                        pdf.set_text_color(0, 0, 0)
                        pdf.multi_cell(0, 8, remainder)
                pdf.ln(8)
            elif line.startswith("- "):
                pdf.multi_cell(0, 8, f"- {line.lstrip('- ').strip()}")
            else:
